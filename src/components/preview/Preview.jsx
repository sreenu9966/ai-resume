...
export function Preview({ resumeRef, isGenerating }) {
    ...
    return (
        <div className="flex flex-col items-center gap-4 h-full relative">
            ...
            {/* Preview Container */}
            <div className={`w-full flex justify-center lg:overflow-hidden transition-all duration-300 h-full ${viewMode === 'pdf'
                ? 'pb-0'
                : ''
                }`}>
                <div className={`${viewMode === 'pdf'
                    ...
                    <ResumePreview ref={resumeRef} viewMode={viewMode} scale={zoom} isEditable={isEditable} />
                </div>
            </div>
        </div>
    );
